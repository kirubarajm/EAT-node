...
exports.update_a_documents = function(req, res) {
    Documentmoveit.updateById(req.params.id, new DocumentSales(req.body), function(err, documents) {
    if (err)
      res.send(err);
    res.json(documents);
  });
 ...
 // console.log(new_documents);
  ...
  //handles null error 
   if(!new_documents || !new_documents_list){
    ...
    Documentmoveit.createnewDocument(new_documents,new_documents_list, function(err, documents) {
    if (err)
      res.send(err);
    res.json(documents);
  });
