...
Eatuser.user_based_notification = async function user_based_notification(req, result) {
  ...
  } else if (req.type==3 && req.coupon.coupon_type==2) {
    
    var getuserquery ="SELECT userid FROM User WHERE created_at >= '"+req.coupon.startdate+"' AND created_at <= '"+req.coupon.enddate+"'";
    
  }else if (req.type==3 && req.coupon.coupon_type==3) {
    
    var getuserquery ="select userid,count(orderid) as cunt  from Orders where  orderstatus=6 and created_at >= '"+req.coupon.startdate+"' AND created_at <= '"+req.coupon.enddate+"' group by userid HAVING cunt > 3";
  }else if (req.type==3 && req.coupon.coupon_type==1) {
