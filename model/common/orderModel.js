...
Order.makeit_logtime = async function makeit_logtime(req) {  
  ///Get Moveit Users list///////
  var makeitlogusersquery = "select makeit_id,date(created_at) as log_date from Makeit_Timelog where date(created_at) between CURDATE()-1 and CURDATE() group by makeit_id order by makeit_id";
  var makeitlogusers = await query(makeitlogusersquery);
  ///Get Moveit Logs///////
  var makeitlogquery = "select date(created_at) as log_date,time(created_at) as logtime,type,makeit_id from Makeit_Timelog where date(created_at) between CURDATE()-1 and CURDATE() order by date(created_at),makeit_id";
