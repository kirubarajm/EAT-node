...
Order.eat_order_item_missing_byuserid = async function eat_order_item_missing_byuserid(req,result) {
  ...
  if (orderdetails) {
    ...
    if (orderdetails[0].orderstatus === 6) {
      ...
      sql.query("UPDATE Orders SET item_missing = 1,item_missing_reason='" +req.item_missing_reason +"',item_missing_by='" +req.item_missing_by +"' WHERE orderid ='" +req.orderid +"'",async function(err, res1) {
          if (err) {
            result(err, null);
          } else {
...
Order.makeit_master_report= function makeit_master_report(req, result) {
    var query="select userid as makeit_id,brandname as kitchen_name,name,phoneno,address,CASE WHEN virtualkey=1 then 'Virtual' WHEN virtualkey=0 then 'Real' END as kitchen_type,date(created_at) as date from MakeitUser where appointment_status=3 and verified_status=1 and ka_status=2 order by userid";
  //console.log("query-->",query);
  sql.query(query,async function(err, res) {
      if (err) {
        result(err, null);
