...
Orderrating.createOrderrating = function createOrderrating(Order_rating, result) {
    ...
    sql.query("Select * from Order_rating where orderid = '"+Order_rating.orderid+"'" , function (err, res) {             
        if(err) {
    ...
    sql.query("INSERT INTO Order_rating set ?", Order_rating, function (err, res) {
        if (err) {
            console.log("error: ", err);
            res(null, err);
        } else {
...
Orderrating.getOrderById = function getOrderById(orderid, result) {
    sql.query("Select * from Order_rating where orderid = ? ", orderid, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Orderrating.updateById = function (id, user, result) {
    console.log('test');
    sql.query("UPDATE Order_rating SET moveit_user_id = ? WHERE orderid = ?", [id, id], function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
        }
...
Orderrating.remove = function (id, result) {
    sql.query("DELETE FROM Order_rating WHERE orderid = ?", [id], function (err, res) {
