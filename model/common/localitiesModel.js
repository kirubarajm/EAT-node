...
Localities.getLocalityById = function getLocalityById(id, result) {
        sql.query("Select * from Locality where localityid = ? ", id, function (err, res) {             
                if(err) {
                    console.log("error: ", err);
                    result(err, null);
                }
...
Localities.updateById = function(id, locality, result){
  sql.query("UPDATE MoveitUser SET Locality = ? WHERE localityid = ?", [locality.task, id], function (err, res) {
          if(err) {
              console.log("error: ", err);
                result(null, err);
             }
...
Localities.remove = function(id, result){
     sql.query("DELETE FROM Locality WHERE localityid = ?", [id], function (err, res) {
