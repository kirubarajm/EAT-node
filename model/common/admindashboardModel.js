...
admindashboardModel.get_all_dashboard_count_by_admin = async function get_all_dashboard_count_by_admin(result) {
  ...
  try {
        ...
        countlist.new_sales_appointment_count  = new_sales_appointment_count;
        //new product count
        ...
        countlist.product_approved_count  = product_approved_count;
        //new  new queries
        var  new_queries_count = await query("Select count(qid) as new_queries_count from Query_questions where admin_read = 1 "); 

        countlist.new_queries_count  = new_queries_count;
          //new_replies_count
        var  new_replies_count = await query("Select count(aid) as new_replies_count from Query_answers where user_read = 1 or admin_read=1"); 
        countlist.new_replies_count  = new_replies_count;
        //new order count
        var  new_order_count = await query("Select count(orderid) as new_order_count from Orders where moveit_user_id = 0 and orderstatus = 0  and cancel_by = 0"); 
        countlist.new_order_count  = new_order_count;
        //makeit order count
