...
ZohoBookModel.getItemIds =function getItemIds(Items,i,zohoitems,result) {
  var session=sessionstorage.getItem("access_token_responce");
  var headers= {
    'Content-Type': 'application/json',
    'Authorization':'Zoho-oauthtoken '+session
  ...
  request.post({headers: headers, url:Contact_url, form: formData,method: 'POST'},async function (e, r, body) {
    ...
    var Jsonvalur= JSON.parse(body);
      if(Jsonvalur.code === 57){
        ZohoBookModel.createZohoRefreshToken(function(){
          ZohoBookModel.getItemIds(Items,i,zohoitems,result);
        });
      }else{
        console.log("reqItems-->",i);
         ...
         // itemIN.rate=item.amount;
          itemIN.quantity=item.quantity;
          itemIN.tax_id="248845000000007208"
        ...
        if(i!==(Items.length-1)){
          ...
          console.log("zohoitems-->",zohoitems);
          result(zohoitems);
        }
...
ZohoBookModel.createZohoItem =async function createZohoItem(req, result) {
   ...
   var line_items=[]
   var i=0;
   console.log("zohoitems-->",reqItems);
    ZohoBookModel.getItemIds(reqItems,i,line_items,function(res){
      req.line_items=res;
      var invoiceReq ={
